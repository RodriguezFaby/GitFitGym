...
DECLARE
    emp_cursor SYS_REFCURSOR;
BEGIN
    -- Llamar a la funci贸n para obtener todos los empleados
    emp_cursor := GET_ALL_EMPLEADOS;

    -- Iterar a trav茅s del cursor y hacer algo con los resultados
    ...
    -- Usar a trav茅s del cursor y hacer algo con los resultados
    FOR emp_rec IN emp_cursor
    LOOP
        DBMS_OUTPUT.PUT_LINE('ID_EMPLEADO: ' || emp_rec.ID_EMPLEADO || ', NOMBRE: ' || emp_rec.NOMBRE);
    END LOOP;
...
-- Leer un empleado espec韋ico por ID
SELECT * FROM EMPLEADO WHERE ID_EMPLEADO = 1;

-- Actualizar informaci髇 de un empleado
UPDATE EMPLEADO
